...
\title{\textbf{Hierarchical Multi-Scale Christoffel Symbolic Mixture for Multi-Resolution Geometry Learning}}
...
\textbf{Keywords:} Multi-scale geometry, Christoffel symbols, hierarchical learning, Riemannian manifolds, scale-space theory, geodesic flow networks
...
\subsection{2.1 Scale-Space Theory}
...
\subsection{2.2 Wavelets and Multi-Resolution Analysis}
...
\subsection{2.3 Low-Rank Christoffel Approximations}
...
\subsection{2.4 Mixture Models in Deep Learning}
...
\section{3. Hierarchical Multi-Scale Christoffel Symbolic Mixture}
...
\subsection{3.2 Scale-Specialized Christoffel Modules}
...
\subsection{3.3 Hierarchical Mixing Architecture}
...
\item \textbf{Parallel scale computation}: For each scale $i$, compute $\Gamma_i(v)$ independently.
\item \textbf{Weight computation}: Apply softmax to mixing logits $\alpha$.
\item \textbf{Weighted combination}: Compute the mixture output as $\sum_i w_i \Gamma_i(v)$.
\end{enumerate}
...
\textit{Proof}: This follows directly from the definition of the mixture and the constraint $\sum_i w_i = 1$. $\square$

\textbf{Proposition 2 (Monotonicity)}: If $r_i < r_j$, then the Lipschitz constant of $\Gamma_i$ is at most that of $\Gamma_j$.

\textit{Proof}: The Lipschitz constant of a low-rank Christoffel decomposition scales with the factor rank, as higher-rank matrices can represent more rapidly varying functions. $\square$
...
\textit{Proof}: This follows from the fact that the set of finite mixtures of functions from the low-rank Christoffel class is dense in the space of continuous functions, under appropriate conditions on the scale ranks. $\square$

\section{4. Implementation Details}

\subsection{4.1 Network Architecture}

The HierarchicalChristoffel module is implemented as a PyTorch \texttt{nn.Module} with the following components:
...
\quad ranks: List[int] = [8, 16, 32],  \# List of ranks for each scale \\
...
The module maintains the following components:
\begin{itemize}
\item \texttt{scales}: ModuleList of LowRankChristoffel modules, one for each rank in \texttt{ranks}
\item \texttt{scale\_weights}: Parameter tensor of shape \texttt{[k]} initialized to uniform values
\end{itemize}

\subsection{4.2 Parallel Scale Computation}

Each scale module is configured to return Christoffel and friction components separately. This enables precise mixing of the two components:

\begin{lstlisting}[style=pystyle]
def forward(self, v, x=None, force=None):
    gammas = []
    mus = []
    
    # Compute all scales in parallel
    for scale in self.scales:
        # Temporarily ensure separate returns
        old_val = getattr(scale, 'return_friction_separately', False)
        scale.return_friction_separately = True
        
        gamma, mu = scale(v, x, force)
        gammas.append(gamma)
        mus.append(mu)
...
\subsection{4.3 Initialization and Training}

The scale modules are initialized with standard random initialization. The mixing weights are initialized to uniform values $\alpha_i = 0$, which corresponds to equal weighting across scales. This initialization allows the model to initially benefit from all scales before learning to emphasize the most relevant ones.

The mixing weights are trained jointly with the Christoffel parameters through backpropagation. No additional loss terms are required, as the softmax parameterization naturally encourages the model to find optimal scale allocations.
...
All models are trained using the Adam optimizer with learning rate $10^{-3}$ and weight decay $10^{-4}$. We use a batch size of 256 and train for 100 epochs. The multi-scale configuration uses ranks $\{8, 16, 32\}$ as specified in the implementation.

\subsection{5.2 Manifold Reconstruction}

\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Method} & \textbf{Reconstruction Error} & \textbf{Parameters} & \textbf{Multi-Scale Gain} \\
\hline
Fixed-Rank 8 & 0.142 & 1.2K & - \\
Fixed-Rank 16 & 0.098 & 2.4K & - \\
Fixed-Rank 32 & 0.071 & 4.8K & - \\
Fixed-Rank 64 & 0.058 & 9.6K & - \\
\textbf{HM-CSM (8,16,32)} & \textbf{0.052} & \textbf{7.2K} & \textbf{+26.8\%} \\
\hline
\end{tabular}
\end{center}
...
\subsection{5.3 Geodesic Distance Estimation}
...
\subsection{5.4 Scale Utilization Analysis}
...
\section*{Appendix A: Scale Selection Guidelines}

We provide practical guidelines for selecting the set of scales $\{r_1, ..., r_k\}$ in HM-CSM:

\begin{enumerate}
\item \textbf{Start with a geometric progression}: Choose ranks $\{r, 2r, 4r, ...\}$ to cover an exponential range of scales.
\item \textbf{Ensure minimum rank}: The smallest rank should be at least 4 to maintain numerical stability.
\item \textbf{Cover the target complexity}: The largest rank should be sufficient to capture the finest geometric details in the data.
\end{enumerate}
...
The computational complexity of HM-CSM with $k$ scales is $\mathcal{O}(k \cdot d \cdot r_{\max})$, where $r_{\max} = \max_i r_i$. For the recommended configuration $\{8, 16, 32\}$ with $k=3$, the complexity is approximately $3 \cdot d \cdot 32 = 96d$, compared to $32d$ for a fixed-rank model with $r=32$. This represents a 3$\times$ increase in computational cost, which is offset by the improved geometric modeling.
