"""
ISN inference: carga checkpoint y genera texto (ISN V5, `test copy/gfn`).
"""

import os
import sys
import json
from pathlib import Path

_GFN_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent.parent
_gfn_root_str = str(_GFN_PROJECT_ROOT)
if _gfn_root_str not in sys.path:
    sys.path.insert(0, _gfn_root_str)

import torch
from gfn import isn


def load_model(checkpoint_path, config_path, vocab_size, device):
    with open(config_path, "r", encoding="utf-8") as f:
        config = json.load(f)

    m = config["model"]
    model = isn.create(
        vocab_size=vocab_size,
        d_model=m["d_model"],
        d_embedding=m["d_embedding"],
        d_properties=m["d_properties"],
        scanner=m.get("scanner", "gfn"),
        world=m.get("world", "gfn"),
        emitter=m.get("emitter", "gfn"),
    ).to(device)

    ckpt = torch.load(checkpoint_path, map_location=device, weights_only=False)
    model.load_state_dict(ckpt["model_state_dict"])
    model.eval()
    print(f"✓ Model loaded from {checkpoint_path} (Loss: {ckpt['loss']:.4f})")
    return model


def main():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    root = _GFN_PROJECT_ROOT
    data_path = root / "tests" / "isn" / "benchmarks" / "language" / "data" / "tinyshakespeare.txt"
    config_path = root / "configs" / "isn" / "shakespeare_config.json"
    checkpoint_path = root / "checkpoints" / "shakespeare" / "best_model.pt"

    if not os.path.exists(checkpoint_path):
        print(f"Error: Checkpoint not found at {checkpoint_path}")
        return

    with open(data_path, "r", encoding="utf-8") as f:
        text = f.read()
    chars = sorted(list(set(text)))
    char_to_ix = {ch: i for i, ch in enumerate(chars)}
    ix_to_char = {i: ch for i, ch in enumerate(chars)}
    vocab_size = len(chars)

    model = load_model(checkpoint_path, config_path, vocab_size, device)

    while True:
        prompt = input("\nEnter prompt (or 'q' to quit): ")
        if prompt.lower() == "q":
            break

        input_ids = torch.tensor(
            [char_to_ix.get(c, 0) for c in prompt], dtype=torch.long
        ).unsqueeze(0).to(device)

        print("\nGenerating...", end="", flush=True)
        generated_ids, _ = model.generate(
            input_ids,
            max_length=200,
            temperature=0.8,
        )

        output_text = "".join(
            [ix_to_char.get(i.item(), "") for i in generated_ids[0]]
        )
        print(f"\n--- Result ---\n{output_text}\n--- End ---\n")


if __name__ == "__main__":
    main()
