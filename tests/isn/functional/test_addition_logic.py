...
def run_addition_demo():
    ...
    with torch.no_grad():
        for i in range(5):
            ...
            # Simple greedy generation for the demo
            # In a real scenario we'd use model.generate()
            outputs = model(input_ids=input_ids, return_world_state=True)
            logits = outputs['logits']
            pred_ids = logits.argmax(dim=-1)
            
            pred_text = generator.detokenize(pred_ids[0].tolist())
            print(f"  Input: {sample['input']} -> Target: {sample['output']} | Pred: {pred_text}")
